class Solution:
    def minimumJumps(self, forbidden: List[int], a: int, b: int, x: int) -> int:
        queue=collections.deque() # (앞으로 갔을 때의 위치, 뒤로 가는 step 수)을 저장하는 queue 선언
        ...
        if a>b: 
            ...
            max_limit=max(x,max(forbidden))+2*b
        ...
        while(len(queue)>0):
            ...
            # 만약 뒤로 갔을 때 그 위치가 0이상이고 방문한 적이 없으며 이전 단계에서 bakward 한적 없고 문제 조건을 만족하면 queue에 저장
                if backward>=0 and backward not in forbidden and backward not in visit and backward<=max_limit and current[1]<1:
                    queue.appendleft([backward, current[1]+1])
                    visit.add(backward)
                # 앞으로 갔을 때 그 위치가 0이상이고 방문한 적이 없으며 문제 조건을 만족하는 경우 queue에 저장
                if forward>=0 and forward not in forbidden and forward not in visit and forward<=max_limit:
